...
def generate_command(
    ...
    package_root = Path(cfg["output_dir"]) / cfg["package_name"]
    dirs_with_modules = {
        p.parent
        for p in generated_paths
        if p.name != "__init__.pyi" and p.parent != package_root
    }

    if any(
        p.parent == package_root and p.name != "__init__.pyi" for p in generated_paths
    ):
        dirs_with_modules.add(package_root)

    for dir_path in dirs_with_modules:
        modules = [p.stem for p in dir_path.glob("*.pyi") if p.name != "__init__.pyi"]
        if not modules:
            continue

        init_path = dir_path / "__init__.pyi"
        lines = ["# generated by typegen\n"]
